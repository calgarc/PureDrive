...
<div id="tabs"> <!--tabs-->
    ...
    <?php
    $username = preg_replace("/[^a-zA-Z0-9\/_|+ .-]/", '', $_SESSION['user']);
    $result = $conn->prepare("SELECT usalt FROM core_users WHERE core_username='".$username."'");
    $result->execute();
    $result->setFetchMode(PDO::FETCH_ASSOC);
    $r = $result->fetch();
    $uid = $r['usalt'];
    ...
    if (ismobile()) {
        ...
        echo '<li class="'.$users.'"><a href="'.$root.'drive/updateuser?id='.$uid.'"><i class="fa fa-users"></i></a></li>';
        }
    }
    ?>
    ...
    echo '<li id="undsh" class="active" style="display:none;"><a href="#" onclick="closedash();"><i class="fa fa-th-large"></i></a></li>';
        ...
        echo '<li class="'.$plugins.'" ><a href="'.$root.'drive/plugins"><i class="fas fa-plug"></i></a></li>';
