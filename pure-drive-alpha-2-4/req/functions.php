...
function newcwd(PDO $conn, $r) {
    ...
    if('Directory' == $oldcwd) {
        $newcwd = subsalt(dirloc($conn, 1).userid($conn, 1).'/'.$cwd.$currentcwd);
    }else {
    ...
    if(isset($_FILES['myFiles'])){
        if('true' == auth($conn, 1)) {
            $errors= array();
                ...
                $result->execute([':id' => id(), ':filesize' => $foldersize]);
                $result->setFetchMode(PDO::FETCH_ASSOC);
            }
    }
...
//function makedir(PDO $conn, $r) {
    ...
    if('true' == auth($conn, 1)) {
        if('drives' == id()) {
            ...
            $source = sanitize($foldername);
            $foldername = encrypt($conn, 1, $source);

        }else {
            $source = subsalt(id()).'/'.str_replace(" ","_",strtolower($_POST['folder']));
            $foldername = encrypt($conn, 1, $source);
...
function active(PDO $conn, $r) {
    ...
    if ('drives' != $r['dir_id']) {
        ...
        $active = subsalt(dirloc($conn, 1).userid($conn, 1 ).'/'.$getid.$r['file_name']);
    }
...
$source =  escape($_GET['name']);
$name = encrypt($conn, 1, $source);
...
<button type="submit" name="renamebtn" class="create renamebtn" value="'.$name.'" onclick="return false;" onmouseover="rnamefiles();"><i class="fas fa-font"></i>rename</button>
    ...
    if (isset($_POST['name'])) {
        ...
        rename(dirloc($conn, 1).$uid.'/'.$newfold , dirloc($conn, 1).$uid.'/'.subsalt(id()).'/'.$newdir);

        $oldcwd = dirloc($conn, 1).$uid.'/'.$newfold;
        $newcwd = dirloc($conn, 1).$uid.'/'.subsalt(id()).'/'.$newdir;
...
function deletefiles(PDO $conn, $r) {
    ...
    if('true' == auth($conn, 1)) {
        ...
        if(isset($_GET['nid'])) {
            ...
            $source = sanitize(escape($_GET['nid']));
            $uid = encrypt($conn, 1, $source);

        }else {

            $source = userid($conn, 1);
            $uid = encrypt($conn, 1, $source);
        }
    }
    ...
    if (isset($_POST['deleted'])) {
        ...
        $new = dirloc($conn, 1).$uid.'/trash/'.subsalt($r['file_name']);
...
function dirlink(PDO $conn, $r) {
    ...
    if('Directory' == $oldcwd) {
        $newcwd = subsalt(dirloc($conn, 1).$uid.'/'.$cwd.'/'.$currentcwd);
...
function gallery(PDO $conn, $r) {
    ...
    while ($r = $result->fetch()) {
        ...
        $source = subsalt($directory.'/'.$r['file_name']);
...
function dispgrid(PDO $conn, $r, $dispfav) {
    ...
    while ($r = $result->fetch()) {
        ...
        $column = 'columng';
        $newcwd = subsalt(dirloc($conn, 1).userid($conn, 1 ).'/'.$r['file_name'].'/'.id());
...
function displist(PDO $conn, $r, $dispfav) {
    ...
    while ($r = $result->fetch()) {
        ...
        if(is_dir(subsalt(dirloc($conn, 1).userid($conn, 1 ).'/'.$current.'/'.$r['file_name']))) {
            $name = str_replace("_"," ",strtolower(subsalt($r['file_name'])));
        }else if(is_file(subsalt(dirloc($conn, 1).userid($conn, 1 ).'/'.$current.'/'.$r['file_name']))) {
        ...
        if(is_dir(subsalt($directory.'/'.$r['file_name']))) {
            ...
            $newcwd = substr(dirloc($conn, 1).userid($conn, 1 ).'/'.$r['file_name'].'/'.$id);
...
function imgdisp(PDO $conn, $r) {
    ...
    $source = subsalt($directory.'/'.$imgmodal);
...
function videodisp(PDO $conn, $r) {
    ...
    $source = subsalt($directory.'/'.$videomodal);
...
function audiodisp(PDO $conn, $r) {
    ...
    $source = subsalt($directory.'/'.$audiomodal);
