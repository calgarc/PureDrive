...
//function makedir(PDO $conn, $r) {
    ...
    $dirloc = $r['setting']."/".userHash($usalt);
...
function displist(PDO $conn, $r, $dispfav) {
    ...
    while ($r = $result->fetch()) {
        ...
        if(is_dir(subsalt(DirLocation($conn, 1).userHash(UserId($conn, 1 )).'/'.$current.'/'.$r['file_name']))) {
            $name = str_replace("_"," ",strtolower(subsalt($r['file_name'])));
        }else if(is_file(subsalt(DirLocation($conn, 1).userHash(UserId($conn, 1 )).'/'.$current.'/'.$r['file_name']))) {
        ...
        if(is_dir(subsalt($directory.'/'.$r['file_name']))) {
            ...
            $newcwd = substr(DirLocation($conn, 1).userHash(UserId($conn, 1 )).'/'.$r['file_name'].'/'.$id);
