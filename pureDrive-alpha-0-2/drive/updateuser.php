...
<div id="right" class="right"><!--right-->
    ...
    $result = $conn->prepare("SELECT core_avatar, core_username, core_email, core_firstname, core_lastname, user_type FROM core_users WHERE usalt='".$usercont."'");
    $result->execute();
    ...
    ?>
        ...
        $source = $_POST['usertype'];
        $usertype = encrypt($conn, 1, $source);


        if (isset($_POST['update'])){
        $result = $conn->prepare("UPDATE core_users SET core_email='".$useremail."', core_firstname='".$userfname."', core_lastname='".$userlname."', user_type='".$usertype."' WHERE usalt='".$usercont."'");
        $result->execute();
        $result->setFetchMode(PDO::FETCH_ASSOC);
        header("Location: updateuser?id=$usercont");
        }
