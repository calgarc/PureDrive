...
function displayfiles(PDO $conn, $r, $dispfav) {
    ...
    $result = $conn->prepare( "UPDATE core_users SET disp_type='$grid' WHERE usalt='".userid($conn, 1)."'");
    $result->execute();
    $result->setFetchMode(PDO::FETCH_ASSOC);
...
function dispgrid(PDO $conn, $r, $dispfav) {
    ...
    $result = $conn->prepare("SELECT COUNT(*) AS count FROM core_folders WHERE user_id='".$uid."' AND dir_id='".$id."'");
    $result->execute();
    $result->setFetchMode(PDO::FETCH_ASSOC);
    $r = $result->fetch();
    $count = $r['count'];

    $result = $conn->prepare("SELECT COUNT(*) AS countf FROM core_files WHERE user_id='".$uid."' AND dir_id='".$id."'");
    $result->execute();
    $result->setFetchMode(PDO::FETCH_ASSOC);
    $r = $result->fetch();
...
function displatest(PDO $conn, $r) {
    ...
    $dispfav = '2';
    if($dispfav == '2') {
    $result = $conn->prepare("SELECT file_name, file_type, cwd FROM core_files WHERE dir_id NOT LIKE 'trash%' AND user_id='".$uid."' ORDER BY reg_date DESC LIMIT 5 ");
    $result->execute();
    $result->setFetchMode(PDO::FETCH_ASSOC);
    }
...
<button type="submit" name="renamebtn" class="create renamebtn" value="'.$name.'" onclick="return false;" onmouseover="rnamefiles();"><i class="fas fa-font"></i>rename</button></div>';
    ...
    if (isset($_POST['name'])) {
        ...
        $newdir = substr($_POST['rename'], 0, -6);
