...
//current id
function id() {
    return preg_replace("/[^a-zA-Z0-9\/_|+ .-]/", '', $_GET['id']);
}
...
function dirlink(PDO $conn, $r) {
    ...
    $uid = preg_replace("/[^a-zA-Z0-9\/_|+ .-]/", '', $_GET['nid']);
    }else {
    $id = preg_replace("/[^a-zA-Z0-9\/_|+ .-]/", '', $_GET['id']);
    }
...
//upload files    
    ...
    $current = preg_replace("/[^a-zA-Z0-9\/_|+ .-]/", '', substr($_GET['id'], 0, -6));
...
//function makedir(PDO $conn, $r) {
    ...
    $source = preg_replace("/[^a-zA-Z0-9\/_|+ .-]/", '', substr($_GET['id'], 0, -6));
...
function gallery(PDO $conn, $r) {
    ...
    $dispfav = '2';
    if($dispfav == '2') {
    $result = $conn->prepare("SELECT file_name, file_type, cwd FROM core_files WHERE file_type LIKE '%image%' AND user_id = :usalt ORDER BY reg_date DESC ");
    $result->execute([':usalt' => $uid]);
    $result->setFetchMode(PDO::FETCH_ASSOC);
    }
...
function dispgrid(PDO $conn, $r, $dispfav) {
    ...
    $sorted = preg_replace("/[^a-zA-Z0-9\/_|+ .-]/", '', $_GET['sorted']);
    }else {
...
<button type="submit" name="renamebtn" class="create renamebtn" value="'.$name.'" onclick="return false;" onmouseover="rnamefiles();"><i class="fas fa-font"></i>rename</button></div>';
    ...
    if (isset($_POST['name'])) {
        ...
        rename(dirloc($conn, 1).$uid.'/'.$newfold , dirloc($conn, 1).$uid.'/'.substr($_GET['id'], 0, -6).'/'.$newdir);
        
        $oldcwd = dirloc($conn, 1).$uid.'/'.$newfold;
        $newcwd = dirloc($conn, 1).$uid.'/'.substr($_GET['id'], 0, -6).'/'.$newdir;
...
function deletefiles(PDO $conn, $r) {
    if (isset($_POST['deleted'])) {
        ...
        $new = substr(dirloc($conn, 1).$r['user_id'].'/trash/'.$r['file_name'], 0, -6);
