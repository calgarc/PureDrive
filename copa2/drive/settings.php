...
$r = $result->fetch();
?>
<label>LOGO</label><span>240x45px recommended size</span>
<div class="uplogo">
...
$r = $result->fetch();
?>
<label>LOGIN BACKGROUND</label><span>1600x1200px recommended size</span>
<div class="uplogo">

<form method="post" enctype="multipart/form-data" id="bgform">
<input type="file" name="background" id="bgfile" class="logoup"><label for="bgfile" style="background:url('<?php echo($r['setting']); ?>') !important; background-size:cover !important; margin:0px !important; height:100% !important; border-radius: 10px;"><i class="fa fa-plus"></i></label>
...
<label>DISPLAY AMMOUNT</label><select class="btn" name="display" id="display"><option value="<?php echo($r['setting'] * 2); ?>"><?php echo($r['setting'] * 2); ?> per page</option><option value="5">10 per page</option>><option value="10">20 per page</option>><option value="25">50 per page</option><option value="50">100 per page</option></select>
...
$source = $_POST['robots'];
$seatch = encrypt($conn, 1, $source);
$result = $conn->prepare("UPDATE core_options SET setting ='".$seatch."'  WHERE options = 'search'");
$result->execute();
$result->setFetchMode(PDO::FETCH_ASSOC);
