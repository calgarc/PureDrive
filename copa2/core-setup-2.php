...
$result = $conn->prepare("CREATE TABLE IF NOT EXISTS core_plugins (id INT(6) UNSIGNED AUTO_INCREMENT PRIMARY KEY, plugin VARCHAR(255) NOT NULL, author VARCHAR(255) NOT NULL, active VARCHAR(255) NOT NULL, url VARCHAR(255) NOT NULL, icon VARCHAR(255) NOT NULL, info VARCHAR(255) NOT NULL, version VARCHAR(255) NOT NULL, reg_date TIMESTAMP)");
$result->execute();
$result->setFetchMode(PDO::FETCH_ASSOC);
...
if ($fail == 0){
$usalt = salted();
$result = $conn->prepare("INSERT INTO core_users (usalt, core_username, core_pass, core_email, salt, core_avatar, user_type, disp_type) VALUES ('".$usalt."','".$_POST["user"]."','".$user_password."','".$_POST["email"]."','".$salted."','".$avatar."','Administrator','listview')");
$result->execute();
$result->setFetchMode(PDO::FETCH_ASSOC);
